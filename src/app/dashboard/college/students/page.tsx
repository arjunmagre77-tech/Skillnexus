...
import { 
  ...
  TrendingUp, Award, CheckCircle2, Eye, Filter, Mail,
  ExternalLink, X, BookOpen, Sparkles
} from "lucide-react";
...
interface StudentRecord {
  ...
  status: "Placed" | "Interviewing" | "Ready" | "Needs Remediation";
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 flex">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <DashboardHeader 
          ...
          subtitle="Real-Time Verified Student Skill Profiles, Academic Performance & Placement Readiness Matrix"
        />
        ...
        <main className="p-6 space-y-6 overflow-y-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="p-5 rounded-2xl bg-slate-900/90 border border-indigo-500/30">
              ...
              <p className="text-xs text-slate-400 mt-1">Verified active profiles</p>
            </div>
            ...
            <div className="p-5 rounded-2xl bg-slate-900/90 border border-emerald-500/30">
              <div className="flex items-center justify-between text-xs text-emerald-400 font-bold uppercase">
                <span>Placement Ready (&gt;80%)</span>
                <ShieldCheck className="w-4 h-4" />
              </div>
              <div className="text-3xl font-black text-white mt-3">{stats.highReadiness}</div>
              <p className="text-xs text-emerald-400/80 mt-1">
                {((stats.highReadiness / stats.total) * 100).toFixed(1)}% of total roster
              </p>
            </div>
            ...
            <div className="p-5 rounded-2xl bg-slate-900/90 border border-cyan-500/30">
              <div className="flex items-center justify-between text-xs text-cyan-400 font-bold uppercase">
                <span>Avg Institutional CGPA</span>
                <TrendingUp className="w-4 h-4" />
              </div>
              <div className="text-3xl font-black text-white mt-3">{stats.avgCgpa}</div>
            ...
            <div className="p-5 rounded-2xl bg-slate-900/90 border border-rose-500/30">
              <div className="flex items-center justify-between text-xs text-rose-400 font-bold uppercase">
                <span>At-Risk (&lt;65% Score)</span>
                <AlertTriangle className="w-4 h-4" />
              </div>
              <div className="text-3xl font-black text-white mt-3">{stats.atRisk}</div>
              <p className="text-xs text-rose-400/80 mt-1">Requires targeted remedial modules</p>
            </div>
          </div>
                      ...
                      <tr key={st.id} className="hover:bg-slate-900/60 transition group">
                        <td className="p-4">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <div className="text-[11px] text-slate-400">{st.rollNo} • {st.email}</div>
                            </div>
                          </div>
                        </td>
                        ...
                        <td className="p-4 min-w-[140px]">
                          <div className="flex items-center justify-between text-[11px] font-bold mb-1">
                            <span className={st.readinessScore >= 80 ? "text-emerald-400" : st.readinessScore >= 65 ? "text-amber-400" : "text-rose-400"}>
                              {st.readinessScore}%
                            </span>
                            <span className="text-slate-400">{st.targetRole}</span>
                          </div>
                        ...
                        <td className="p-4">
                          {st.status === "Placed" && (
                            <span className="px-2.5 py-1 rounded-full bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 font-bold text-[10px] flex items-center gap-1 w-fit">
                              <CheckCircle2 className="w-3 h-3" /> Placed ({st.companyPlaced})
                            </span>
                          )}
                          {st.status === "Ready" && (
                        ...
                        <td className="p-4 text-right">
                          <button 
                            ...
                            className="px-3 py-1.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-cyan-400 font-bold text-xs transition flex items-center gap-1 ml-auto"
                          >
        ...
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl max-w-lg w-full p-6 space-y-6 relative shadow-2xl">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {activeStudent.missingSkills.map((s) => (
                      <span key={s} className="px-2.5 py-1 rounded-lg bg-rose-500/20 text-rose-400 border border-rose-500/30 font-bold">
                        ⚠️ Missing: {s}
                      </span>
                    ))}
                  </div>
            ...
            <div className="pt-4 border-t border-slate-800 flex gap-3">
              <button 
                onClick={() => {
                  showToast(`Assigned target learning module to ${activeStudent.name}`);
